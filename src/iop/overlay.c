...
{
  DT_OVERLAY_COMPOSITE_HQ = 0,    // $DESCRIPTION: "high-precision (v1)"
  DT_OVERLAY_COMPOSITE_LEGACY = 1 // $DESCRIPTION: "legacy 8bit (v0)"
...
typedef struct dt_iop_overlay_params_t
{
  ...
  // compositing precision. New instances default to HQ (float); edits made
  // before this option existed are migrated to LEGACY by legacy_params() so
  // they keep their original appearance.
  dt_iop_overlay_compositing_t
    compositing; // $DEFAULT: DT_OVERLAY_COMPOSITE_HQ $DESCRIPTION: "compositing"
...
typedef struct dt_iop_overlay_gui_data_t
{
  ...
  GtkWidget *rotate;
  GtkWidget *compositing; // compositing precision
  GtkWidget *imgid;
...
void gui_init(dt_iop_module_t *self)
{
  ...
  // other control is rendered, and pins the look of pre-existing edits.
  g->compositing = dt_bauhaus_combobox_from_params(self, "compositing");
  gtk_widget_set_tooltip_text(
    g->compositing,
    _("compositing algorithm\n"
      "• high-precision (v1): composite in linear floating point — no banding,\n"
      "  at the cost of a larger overlay cache (16 bytes/pixel)\n"
      "• legacy 8bit (v0): original 8-bit compositing — lower memory footprint\n"
      "  (4 bytes/pixel) but visible banding on smooth gradients; kept so edits\n"
      "  created before the high-precision path look unchanged"));
