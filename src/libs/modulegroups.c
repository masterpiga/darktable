...
static void _cycle_module_groups(const gboolean down, dt_lib_module_t *self)
{
  ...
  if(down)
  {
    if(d->current == DT_MODULEGROUP_BASICS)
      ...
      next = ngroups > 0 ? _buttons_get_from_pos(self, 1)
                         : (has_basics ? d->basic_btn : NULL);
    else
    {
      next = _buttons_get_from_pos(self, d->current + 1);
      if(!next) next = has_basics ? d->basic_btn : d->active_btn;
    }
  }
  else
  {
  ...
  if(next)
  {
    ...
    dt_toast_log(N_("module group: [%s]"), _get_current_group_name(self));
  }
}
                                                  ...
                                                  const float move_size)
{
  ...
  if(DT_PERFORM_ACTION(move_size))
  {
    if(effect == DT_ACTION_EFFECT_DEFAULT_KEY)
    {
      ...
      _cycle_module_groups(effect == DT_ACTION_EFFECT_DOWN, self);
    return 0;
  }
...
void gui_init(dt_lib_module_t *self)
{
  ...
  // cycle module groups action
  dt_action_define(DT_ACTION(self), NULL, N_("cycle module groups"),
