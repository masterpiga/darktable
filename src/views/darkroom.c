...
// was last left, and is what re-focuses it on the way back in. It held nothing
// but the operation name, which cannot tell two instances of one module apart:
// the restore then focused whichever instance came last in dev->iop, while the
// expansion state went to instance 0, leaving one instance expanded and a
// different one focused. In accordion mode (darkroom/ui/single_module) that is
// a state the user has no way to produce by hand.
//
// Instances past the first are now qualified with their multi_priority, which
